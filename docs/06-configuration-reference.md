...
| `handoff_max_chars` | `checkpoint_handoff_max_chars` | `8000` | Maximum characters of Claude's `compact_summary` stored as the checkpoint handoff by `PostCompact`. Only the `<summary>` block of Claude's raw compaction output is kept (the `<analysis>` scratch block is discarded), then the text is secret-stripped and truncated with a `…[truncated]` marker. Minimum 400; lower values are rejected at load time. |
