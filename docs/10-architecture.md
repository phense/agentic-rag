...
| `PostCompact` | `manual\|auto`, 3s | `writer` | Selects the newest same-session/same-trigger `PreCompact` checkpoint (turn-less match; compacted or not — the newest compaction wins), marks the boundary, and stores the `<summary>` block of the payload's `compact_summary` (the `<analysis>` scratch block is discarded; boundaries are tags on lines of their own) as the bounded (head and tail kept around `…[truncated]`), secret-stripped **handoff** (`store.attach_handoff`, audited). Never emits `additionalContext`. | **Fail open** — no match is a silent no-op; a DB failure emits only a `systemMessage`. |
...
`CURRENT`/`HISTORICAL` age label (shortened into the remaining render budget,
head and tail kept, rather than dropped whole) and caps its whole `additionalContext` at
`context_max_chars` (default 9,500; hard maximum 10,000, Claude's per-hook
limit). The checkpoint is the elastic section: it is re-rendered into the
remaining budget before any whole section is dropped; only then are recent
knowledge, domains, checkpoint, and pins trimmed, with a visible warning that
names the shortened section, the dropped ones, and the pins cut. Claude Code
starts `SessionStart(source="compact")` and `PostCompact` concurrently, so
the injection right after a compaction usually predates the handoff; the
handoff serves the next startup/resume and `rag status`.
...
PostCompact ──► latest_pre_compact() → mark_compacted() → attach_handoff(<summary> of compact_summary)
SessionStart(source="compact")          (runs concurrently with PostCompact — handoff usually not yet stored)
   └─ latest_for_session() → render_checkpoint() → fit_context(≤ context_max_chars)
SessionStart(source="startup"|"resume")   (next session: handoff present)
   └─ latest_for_session()/latest_for_project() → render_checkpoint(+handoff, elastic) → fit_context
```
