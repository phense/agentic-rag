...
| `PostCompact` | Silent on success. Matches the newest same-session/same-trigger `PreCompact` checkpoint without a `turn_id` (compacted or not — the newest compaction wins), marks it compacted, and stores the `<summary>` block of the payload's `compact_summary` (the `<analysis>` scratch block is discarded) as the bounded, secret-stripped handoff (`handoff_max_chars`, default 8,000). Never emits `additionalContext`; no match is a no-op; a DB failure emits only `{"systemMessage":"checkpoint bookkeeping delayed"}`. |
| `SessionStart(source="compact")` | Emits the same-session checkpoint as `additionalContext`. Claude Code starts this hook and `PostCompact` concurrently, so the injection right after a compaction normally predates the handoff (Claude's own summary is still in context); the `Handoff (Claude compact summary, CURRENT|HISTORICAL, age=…h)` section appears on the next `startup`/`resume` injection. The whole output is capped at `context_max_chars` (default 9,500; Claude discards per-hook context above 10,000 characters): the checkpoint is shortened into the remaining budget first, whole sections are dropped only after that, and the output starts with a `⚠️ context truncated …` warning naming what was shortened, dropped, or cut. Same selection rules as Codex; compact never falls back to another session or project. |
