...
**hooks → queue → worker → `claude -p` → gateway.** A `Stop` hook fires when a Claude Code session ends and enqueues that session's transcript. A single-writer background worker drains the queue and, for each job, runs `claude -p` against whatever your local `claude` CLI is authenticated with — your Claude subscription or an API key — to extract durable memories, lessons, and signals from the session digest — each candidate item is checked for a near-duplicate against what's already stored (an embedding-similarity gate) before it's written, and every extracted item still goes in through `save_document()`, same as a manual save. The miner can also flag contradictions with existing documents or pins, and propose new domains — those surface for you to review rather than mutating anything unattended.
