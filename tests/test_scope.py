...
def test_additive_legacy_migration_and_reader_privileges(cfg,tmp_path,monkeypatch):
    ...
    with isolated_database(cfg) as isolated:
        with db.connect(isolated,role='owner') as connection:
            ...
            connection.commit()
            assert db.apply_migrations(connection,db.SQL_DIR)==['010_project_scope.sql', '011_fact_validity.sql', '012_claim_evidence.sql', '013_retrieval_quality.sql', '014_project_profiles.sql']
