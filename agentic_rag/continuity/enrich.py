...
SYSTEM = (
    ...
    "delta; never infer that a test passed. Processes and external states must "
    "include only observations explicitly present in the delta, phrased as "
    "past observations; never infer current process liveness or current "
    "external state. Use empty strings or arrays when evidence is absent. "
    "rag_slugs may contain only slugs literally present in the delta."
)
...
def enrich_checkpoint(conn, cfg: Config, job: dict, runner) -> str | None:
